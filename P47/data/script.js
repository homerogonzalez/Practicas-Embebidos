...
para cada punto de datos en una serie en el gráfico. */
        ...
        series: {
            color: '#9147ff' // color de la serie de puntos para humedad
        }
    },
    ...
    yAxis: {
        title: {
            text: 'Temperatura en grados centígrados (°C)', // etiqueta vertical de la gráfica
            style: {
                color: '#FFFFFF'
            }
        },
...
En nuestro ejemplo, queremos colocarlo en la etiqueta <div>
con el ID grafica-temperatura (se vinclula en index.html) */
        renderTo: 'grafica-presion', /* La gráfica es colocada en el elemento HTML
con identificador 'grafica-temperatura' */
        backgroundColor: '#18181b'
    },
    // se define el nombre de las series
...
para cada punto de datos en una serie en el gráfico. */
    ...
    yAxis: {
        title: {
            text: 'Presión (hPa)', // etiqueta vertical de la gráfica
            style: {
                color: '#FFFFFF'
            }
        },
...
En nuestro ejemplo, queremos colocarlo en la etiqueta <div>
con el ID grafica-temperatura (se vinclula en index.html) */
        renderTo: 'grafica-altitud', /* La gráfica es colocada en el elemento HTML
con identificador 'grafica-temperatura' */
        backgroundColor: '#18181b'
    },
    // se define el nombre de las series
...
para cada punto de datos en una serie en el gráfico. */
    ...
    yAxis: {
        title: {
            text: 'Altitud sobre el nivel del mar (metros)', // etiqueta vertical de la gráfica
            style: {
                color: '#FFFFFF'
            }
        },
...
function graficarPresion(value) { /* para agregar un punto al gráfico,
usamos las siguientes líneas: */
    var x = (new Date()).getTime() // El valor 'x' del gráfico es la marca de tiempo.
    var y = Number(value); /*El valor de 'y' se pasa como argumento. Necesitamos
convertirlo en un número. */
    if (chartP.series[0].data.length > 40) { /* Nuestros gráficos solo tienen una serie (el índice es 0). Entonces,
podemos acceder a la serie en el gráfico de temperatura utilizando:
chartT.series [0].
Primero, verificamos la longitud de los datos de la serie:
• Si la serie tiene más de 40 puntos: agregar y cambiar un nuevo punto;
• O si la serie tiene menos de 40 puntos: agregar un nuevo punto.
*/
        chartP.series[0].addPoint([x, y], true, true, true); // últimos argumentos: Redraw,shift,
        // withEvent
        /* Para agregar un nuevo punto, se usa el método addPoint ()
        que acepta lo siguientes argumentos:
        El valor que se trazará. Si es un solo número, un punto con ese valor y es
        ...
        */
        ...
        chartP.series[0].addPoint([x, y], true, false, true); // últimos argumentos: Redraw,shift,
        //withEvent
        /* • Opción Redraw -Redibujar- (booleano): establezca en true para volver a dibujar
        el gráfico después de que el punto sea adicional.
        • Opción de corrimiento -shift- (booleano): si es falso, un punto NO se desplaza del
...
function graficarAltitud(value) { /* para agregar un punto al gráfico,
usamos las siguientes líneas: */
    var x = (new Date()).getTime() // El valor 'x' del gráfico es la marca de tiempo.
    var y = Number(value); /*El valor de 'y' se pasa como argumento. Necesitamos
convertirlo en un número. */
    if (chartA.series[0].data.length > 40) { /* Nuestros gráficos solo tienen una serie (el índice es 0). Entonces,
podemos acceder a la serie en el gráfico de temperatura utilizando:
chartT.series [0].
Primero, verificamos la longitud de los datos de la serie:
• Si la serie tiene más de 40 puntos: agregar y cambiar un nuevo punto;
• O si la serie tiene menos de 40 puntos: agregar un nuevo punto.
*/
        chartA.series[0].addPoint([x, y], true, true, true); // últimos argumentos: Redraw,shift,
        // withEvent
        /* Para agregar un nuevo punto, se usa el método addPoint ()
        que acepta lo siguientes argumentos:
        El valor que se trazará. Si es un solo número, un punto con ese valor y es
        ...
        */
        ...
        chartA.series[0].addPoint([x, y], true, false, true); // últimos argumentos: Redraw,shift,
        //withEvent
        /* • Opción Redraw -Redibujar- (booleano): establezca en true para volver a dibujar
        el gráfico después de que el punto sea adicional.
        • Opción de corrimiento -shift- (booleano): si es falso, un punto NO se desplaza del
...
function obtenerLecturas() {
    ...
    variableAJAX.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            var pres = miObjetoJSON.presion;
            var altu = miObjetoJSON.altitud;
            ...
            graficarHumedad(hum);
            graficarPresion(pres);
            graficarAltitud(altu);
        }
    };
    ...
    fuenteDeEventos.addEventListener
        ('nuevas_lecturas', function (e) {
            ...
            */
            ...
            graficarPresion(miObjetoJSON.presion);
            graficarAltitud(miObjetoJSON.altitud);
